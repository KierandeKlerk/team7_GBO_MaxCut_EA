...
def run_genetic_algorithm_eval_optimization(run_params):
    inst, evaluation_budget, cx, n_runs = run_params
...
def eval_optimization(set):
    ...
    for cx in crossovers:
        file_out = "output-num_eval-{}_{}.csv".format(cx, currentDT.strftime("%d-%m-%Y_%H-%M"))
        with open(os.path.join(dirOut,file_out),"w") as f:
            f.write("dimensionality,num_edges,population_size,success,median_num_eval\n")
            
            run_params = [(inst, evaluation_budget, cx, n_runs) for inst in filesIn[0::10] for evaluation_budget in [10, 100, 1000, 5000, 10000, 50000, 100000, 200000, 500000, 750000, 1000000]]
            
            pool = mp.Pool()  # creates a pool of worker processes
            results = pool.map(run_genetic_algorithm_eval_optimization, run_params)
            for i, result in enumerate(results):
                success, median_num_evaluations, dimensionality, num_edges, population_size = result
                print("\nDimensionality: {}, Median_num_evaluations: {}, Success, {} \n".format(dimensionality, median_num_evaluations, success))
